...
from modules.summarize.schemas import SummarizeRequest, SummarizeResponse
from modules.summarize.service import execute_summarize
from modules.jobs.service import enqueue_async_job

router = APIRouter(prefix="/v1/ai/summarize", tags=["Summarize"])


@router.post(
    "",
    dependencies=[Depends(require_permission("summarize"))]
)
async def summarize_endpoint(
    ...
    Supports dual response mode:
    1. Synchronous (default): Executes LLM call directly, returning summarized text (HTTP 200).
    2. Asynchronous (X-Response-Mode: async): Enqueues job to Redis arq queue, returning job_id (HTTP 202).
    """
    # 1. Handle Asynchronous Response Mode (Section 6.3 & Section 8.4)
    ...
    return await execute_summarize(
        request=request,
        app=app,
        db=db,
        user_id=x_user_id,
